...
<div class="container-fluid">
        <br/>
        <form class="form-horizontal" role="form" method="post" name="jqx-form" id="jqx-form-add-schedule">
            {{ csrf_field() }}
            <div class="row">
                <div class="col-md-6">
    ...
    <script>
        $(document).ready(function(){
            ...
            function getBusy(){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        if(data.rooms){
                            var busy =  JSON.parse(data.rooms);
                            var i = 0;
                            for(let key in rooms){
                                $("#room").jqxDropDownList('enableAt', i);
                                for(let k in busy){
                                    if(busy[k] == key){
                                        $("#room").jqxDropDownList('clearSelection'); 
                                        $("#room").jqxDropDownList('unselectItem', i ); 
                        ...
                        if(data.trainers){
                            ...
                            for(let key in trainers){
                                ...
                                for(let k in busy){
                                    if(busy[k] == key){
                                        $("#trainer").jqxDropDownList('unselectItem', i ); 
                        ...
                        if(data.assistants){
                            ...
                            for(let key in assistants){
                                ...
                                for(let k in busy){
                                    if(busy[k] == key){
                                        $("#assistant").jqxDropDownList('unselectItem', i ); 
            ...
            $("#btn-cancel-schedule").click(function(){
                $('#jqxwindowadd_schedule').jqxWindow('destroy');
                loadSchedule();
            });

            $("#jqxwindowadd_schedule").on('close',function(){
                loadSchedule();
            });

            $("#btn-save-schedule").click(function(){
                var formData = new window.FormData($('#jqx-form-add-schedule')[0]);

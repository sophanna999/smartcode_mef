...
<div class="container-fluid">
		...
		<form class="form-horizontal" role="form" method="post" name="jqx-form" id="jqx-form">
			...
			<div class="form-group">
				...
				<div class="col-sm-7">
					<input type="text" name="code" value="{{ $room->code }}" class="form-control" id="code" placeholder="{{ trans('general.enter',['attribute' => trans('room.code') ]) }}">
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-7">
					<input type="text" name="name" value="{{ $room->name }}" class="form-control" id="name" placeholder="{{ trans('general.enter',['attribute' => trans('room.name') ]) }}">
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-7">
					<textarea name="description" class="form-control" id="description" placeholder="{{ trans('general.enter',['attribute' => trans('room.description') ]) }}" cols="30" rows="5">{{ $room->description }}</textarea>
				</div>
			</div>
			...
			@if(isAdmin())
				{{-- <div class="form-group">
					...
					<div class="col-sm-7">
						<input name="public" {{ $room->public ? 'checked' : '' }} class="margin-top-lg" value="1" type="checkbox"/>
					</div>
				</div> --}}
			@endif
	...
	<script>
		$(document).ready(function(){
			...
			initDropDown('location',{!! $locations !!},{{ $room->location->id }});

			$("#jqx-save").click(function(){
				saveJqxItem('', '{{ secret_route() }}/room/' + {{ $room->id }}, '{{ csrf_token() }}');
			});

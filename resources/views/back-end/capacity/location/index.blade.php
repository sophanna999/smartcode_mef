...
@section('content')
        ...
        <div class="panel">
            <div class="row panel-heading custome-panel-headering">
                <div class="form-group title-header-panel">
                    <div class="pull-left">
                    <div class="col-lg-12">{{trans('general.capacity_building')}} &raquo;{{trans('location.location')}}</div>            
                </div>
    ...
    <script type="text/javascript">
        var source =
        {
            ...
            cache: false,
            id: 'id',
            url: '{{ secret_route() }}/location/lists',
            ...
            deleteRow: function (rowid, commit) {
                ajaxPostRequest('{{ secret_route()}}/location',{ _method:'DELETE',id:rowid});
            }
        };
        ...
        $(document).ready(function () {
            ...
            $("#jqx-grid").jqxGrid({
                    ...
                    columns: [
                        { text: '{{ trans('location.code') }}', dataField: 'code'},
                        { text: '{{ trans('location.name') }}', dataField: 'name'},
                        // { text: '{{ trans('department.department') }}', dataField: 'department'},
                        { text: '{{ trans('location.description') }}', dataField: 'description'},
                    ]
            });

            $("#btn-new").on('click',function(){
                newJqxItem('', '{{$constant['buttonNew']}}',700,380, '{{ secret_route() }}/location/create');
            });
            ...
            $("#btn-edit").on('click',function(){
                ...
                if(!row.length){
                    ...
                    newJqxItem('', '{{$constant['buttonEdit']}}', 700, 380, '{{ secret_route() }}/location/edit/'+ jqxdatarow.uid);
                }
            });

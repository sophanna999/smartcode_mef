...
<div class="container-fluid">
    ...
    <form class="form-horizontal" role="form" method="post" name="jqx-form" id="jqx-form">
        ...
        <div class="row">
            <div class="col-sm-6">
                <div class="form-group">
                    ...
                    <div class="col-sm-7">
						<input type="text" value="{{ $survey_template->code }}" name="code" class="form-control" id="code" placeholder="{{ trans('general.enter',['attribute' => trans('survey.code') ]) }}">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-7">
                        <input type="text" value="{{ $survey_template->name }}" name="name" class="form-control" id="name" placeholder="{{ trans('general.enter',['attribute' => trans('survey.name') ]) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="col-sm-6">
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <textarea name="description" class="form-control" id="description" placeholder="{{ trans('general.enter',['attribute' => trans('survey.description') ]) }}" cols="30" rows="3.5">{{ $survey_template->description }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <table class="table" id="question_table">
            ...
            <tbody>
				@foreach($survey_template->question as $question)
					<tr class="active">
						<td><input type="hidden" name="question[]" value="{{ $question->id }}"/></td>
						<td>{{ $question->title }} <span class="label label-info pull-right">{{ config('system.answer_types.'.$question->type) }}</span> 
						@if($question->type == 1 || $question->type == 2 || $question->type == 3)
							<ul>
								@foreach(explode(',',$question->option) as $option)
									<li>{{ $option }}</li>
								@endforeach
							</ul>
						@endif
						</td>
						<td><a class="remove_question" href="javascript:void(0)"><i class="fa fa-remove text-danger"></i></a></td>
					</tr>
				@endforeach
			</tbody>
        </table>
...
<script>
    $(document).ready(function(){
        ...
        $("#jqx-save").click(function(){
            saveJqxItem('', '{{ secret_route() }}/survey-template/' + {{ $survey_template->id }}, '{{ csrf_token() }}',1,function(request){
                if(request.status == 422 && response['question']){

...
$newUrl = asset($constant['secretRoute'].'/department/new');
$editUrl = asset($constant['secretRoute'].'/department/edit');
$listUrl = asset($constant['secretRoute'].'/department/index');
$deleteUrl = asset($constant['secretRoute'].'/department/delete');
?>
		...
		<div class="panel">
            <div class="row panel-heading custome-panel-headering">
                <div class="form-group title-header-panel">
                    <div class="pull-left">
                    <div class="col-lg-12">{{trans('users.systemSetting')}} &raquo; {{trans('officer.department')}}</div>
                </div>
...
<script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#jqx-grid<?php echo $jqxPrefix;?>").jqxGrid({
                    ...
                    columns: [
                        { text: '{{trans('officer.centralMinistry')}}', dataField: 'mef_ministry_id', width: '28%',filterable: false, hidden:true },
                        { text: '{{trans('officer.generalDepartment')}}', dataField: 'mef_secretariat_id', width: '30%',filterable: false, hidden:true },
						{ text: '{{trans('officer.department')}}', dataField: 'Name', width: '80%' },
						{ text: '{{$constant['abbreviation']}}', dataField: 'Abbr', width: '18.5%' }
                    ],
                    showgroupsheader: false,
                    groupable:true,
                    groupsexpandedbydefault: true,
                    groups: ['mef_ministry_id','mef_secretariat_id']
            });

           
            $("#btn-new<?php echo $jqxPrefix;?>").on('click',function(){
                newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonNew']}}',600,300, '<?php echo $newUrl;?>', 0, '{{ csrf_token() }}');
            });
            ...
            $("#btn-edit<?php echo $jqxPrefix;?>").on('click',function(){
                ...
                if(row.length == 0){
                    ...
                    newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonEdit']}}', 600, 300, '<?php echo $editUrl;?>', jqxdatarow.Id, '{{ csrf_token() }}');
                }

...
<div class="container-fluid">
	<div class="row search-options">
		<div class="col-lg-1 col-sm-1 col-md-2 text-right labels-door-tracking">{{trans('attendance.dayMonthYear')}}</div>
		<div class="col-lg-2 col-sm-2 col-md-2">
			<input type="hidden" id="from-date-value" value="">
			<div id="jqx-from-date"></div>
		</div>
		<div class="col-lg-1 col-sm-1 col-md-2 text-right labels-door-tracking">{{trans('attendance.OfficerName')}}</div>
		<div class="col-lg-2 col-sm-2 col-md-3">
			<input type="hidden" id="officer_id" value="">
			<div id="mef_officer_id"></div>
		</div>
		<div class="col-lg-1 col-sm-1 col-md-1">
			<button id="btn-search" type="button"><span class="glyphicon glyphicon-search"></span> {{trans('trans.buttonSearch')}}</button>
		</div>
		<div class="col-lg-1 col-sm-1 col-md-1">
			<button id="btn-report" type="button"><span class="glyphicon glyphicon-download-alt"></span> {{trans('attendance.advance_search')}}</button>
		</div>
	</div><br>
...
<script type="text/javascript">
	...
	function initJqxWindow(date,officer_id) {
        ...
        newJqxAjax('_tracking', '{{trans('officer.detail')}}',width, height - 100, url, param);
    }
	function getDetail(){
	...
	$(document).ready(function () {
		var buttons = ['btn-search','btn-report'];
        ...
        $('#btn-report').on('click',function(){
            var url = '{{asset($constant['secretRoute'].'/door-tracking/report')}}';
            var param = {
                _token:'{{csrf_token()}}'
            };
            newJqxAjax('_tracking', '{{trans('trans.report')}}',width, height, url, param);
        });

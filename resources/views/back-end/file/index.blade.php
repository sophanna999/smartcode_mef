<?php
$jqxPrefix = '_file';
$listUrl = asset($constant['secretRoute'].'/file/index');
$newUrl = asset($constant['secretRoute'].'/file/new');
$editUrl = asset($constant['secretRoute'].'/file/edit');
$deleteUrl = asset($constant['secretRoute'].'/file/delete');
?>
		...
		<div class="panel">
            <div class="row panel-heading custome-panel-headering">
                <div class="form-group title-header-panel">
                    <div class="pull-left">
                        <div class="col-lg-12 col-xs-12">{{trans('file.file_management')}} &raquo; {{trans('file.directory_folder')}}</div>
                    </div>
                    <div class="pull-right">
                        <div class="col-lg-12 col-xs-12">
							<button id="btn-new<?php echo $jqxPrefix;?>"><i class="glyphicon glyphicon-plus"></i> {{trans('trans.buttonNew')}}</button>
							<button id="btn-edit<?php echo $jqxPrefix;?>"><i class="glyphicon glyphicon-edit"></i> {{trans('trans.buttonEdit')}}</button>
							<button id="btn-delete<?php echo $jqxPrefix;?>"><i class="glyphicon glyphicon-trash"></i> {{trans('trans.buttonDelete')}}</button>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$("#jqx-grid<?php echo $jqxPrefix;?>").jqxGrid({
				...
				columns: [
                    { text: '{{trans('trans.autoNumber')}}', dataField: 'id', width: '5%',align:'center',columntype: 'number',cellsrenderer:numberRenderer,cellsalign: 'center',editable: false },
					{ text: '{{trans('file.directory_folder')}}', dataField: 'name', width: '20%',editable: false},
                    {text: '{{trans('file.file_size')}}', dataField: 'file_size', width: '7%',editable: false},
                    {text: '{{trans('file.file_type')}}', dataField: 'file_type', width: '7%',editable: false},
                    {text: '{{trans('trans.status')}}', dataField: 'status', width: '7%',editable: false},
					{text: '{{trans('file.file_last_modified')}}', dataField: 'last_modified', width: '10%',editable: false},
                    {text: '{{trans('file.file_link')}}', dataField: 'file_path', width: '44%',editable: true}
				]
		});

	   
		$("#btn-new<?php echo $jqxPrefix;?>").on('click',function(){
			newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonNew']}}',500,260, '<?php echo $newUrl;?>', 0, '{{ csrf_token() }}');
		});
		...
		$("#btn-edit<?php echo $jqxPrefix;?>").on('click',function(){
			...
			if(row.length == 0){
				...
				newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonEdit']}}',500,260, '<?php echo $editUrl;?>', jqxdatarow.Id, '{{ csrf_token() }}');
			}

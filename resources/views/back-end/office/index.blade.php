...
$newUrl = asset($constant['secretRoute'].'/office/new');
$editUrl = asset($constant['secretRoute'].'/office/edit');
$listUrl = asset($constant['secretRoute'].'/office/index');
$deleteUrl = asset($constant['secretRoute'].'/office/delete');
?>
		...
		<div class="panel">
            <div class="row panel-heading custome-panel-headering">
                <div class="form-group title-header-panel">
                    <div class="pull-left">
                    <div class="col-lg-12">{{trans('users.systemSetting')}} &raquo; {{trans('officer.office')}}</div>
                </div>
...
<script type="text/javascript">
        ...
        var source<?php echo $jqxPrefix;?> =
        {
            ...
            dataFields: [
				...
				{ name: 'mef_secretariat_id', type: 'string' },
				{ name: 'mef_ministry_id', type: 'string' },
				{ name: 'mef_department_id', type: 'string' }
            ],
			cache: false,
            id: 'id',
        ...
        $(document).ready(function () {
            ...
            $("#jqx-grid<?php echo $jqxPrefix;?>").jqxGrid({
                    ...
                    columns: [
                        { text: '{{trans('officer.centralMinistry')}}', dataField: 'mef_ministry_id', width: '25%',filterable:false, hidden:true},
                        { text: '{{trans('officer.generalDepartment')}}', dataField: 'mef_secretariat_id', width: '20%', hidden:true },
						{ text: '{{trans('officer.department')}}', dataField: 'mef_department_id', width: '18%', hidden:true },
						{ text: ' {{trans('officer.office')}}', dataField: 'Name', width: '80%' },
						{ text: '{{$constant['abbreviation']}}', dataField: 'Abbr', width: '18.5%' }
                    ],
                    showgroupsheader: false,
                    groupable:true,
                    groupsexpandedbydefault: true,
                    groups: ['mef_ministry_id','mef_secretariat_id','mef_department_id']
            });

           
            $("#btn-new<?php echo $jqxPrefix;?>").on('click',function(){
                newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonNew']}}',600,370, '<?php echo $newUrl;?>', 0, '{{ csrf_token() }}');
            });
            ...
            $("#btn-edit<?php echo $jqxPrefix;?>").on('click',function(){
                ...
                if(row.length == 0){
                    ...
                    newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonEdit']}}',600, 370, '<?php echo $editUrl;?>', jqxdatarow.Id, '{{ csrf_token() }}');
                }

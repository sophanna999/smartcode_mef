...
<div class="container-fluid">
    <form action="<?php echo $exportUrl; ?>" class="form-horizontal" role="form" method="post" name="jqx-form<?php echo $jqxPrefix;?>" id="jqx-form<?php echo $jqxPrefix;?>" enctype="multipart/form-data">
		...
		<div class="form-group">
			<div class="col-sm-5 text-right"><span class="red-star">*</span> {{trans('schedule.meeting_participant')}}</div>
			<div class="col-sm-7">
				<input type="hidden" id="officer_name" name="officer_name" value="" />
				<div id="div_officer_id" name="officer_id"></div>
			</div>
        </div>
        <div class="form-group">
...
<script>
    $(document).ready(function(){
		...
		?>
		// div_officer_id
        initDropDownList(jqxTheme, 204,36, '#div_officer_id', <?php echo $mef_officer_department; ?>, 'text', 'value', false, '', '0', "#officer_id","{{$constant['buttonSearch']}}",250);
		//On dropdown select event
		$("#div_officer_id").bind('select', function (event) {
			var args = event.args;
			var item = $("#div_officer_id").jqxDropDownList('getItem', args.item);
			if(item == undefined){
				$('#officer_name').val('');
			}else{

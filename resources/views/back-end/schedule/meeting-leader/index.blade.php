...
$newUrl = asset($constant['secretRoute'].'/meeting-leader/new');
$editUrl = asset($constant['secretRoute'].'/meeting-leader/edit');
$listUrl = asset($constant['secretRoute'].'/meeting-leader/index');
$deleteUrl = asset($constant['secretRoute'].'/meeting-leader/delete');
?>
		...
		<div class="panel">
            <div class="row panel-heading custome-panel-headering">
                <div class="form-group title-header-panel">
                    <div class="pull-left">
                    <div class="col-lg-12">{{trans('schedule.schedule')}} &raquo; {{trans('schedule.meeting_leader')}}</div>
                </div>
...
<script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#jqx-grid<?php echo $jqxPrefix;?>").jqxGrid({
                    ...
                    columns: [
                        { text: '{{trans('schedule.meeting_leader')}}', dataField: 'Name', width: '50%'},
                        { text: '{{trans('schedule.email')}}', dataField: 'email', width: '35%'},
            ...
            $("#btn-new").on('click',function(){
                newJqxItem('<?php echo $jqxPrefix;?>', '{{trans('trans.buttonNew')}}',500, 300, '<?php echo $newUrl;?>', 0, '{{ csrf_token() }}');
            });
            ...
            $("#btn-edit").on('click',function(){
                ...
                if(row.length == 0){
                    ...
                    newJqxItem('<?php echo $jqxPrefix;?>', '{{trans('trans.buttonEdit')}}', 500, 300, '<?php echo $editUrl;?>', jqxdatarow.Id, '{{ csrf_token() }}');
                }

...
$newUrl = asset($constant['secretRoute'].'/meeting-room/new');
$editUrl = asset($constant['secretRoute'].'/meeting-room/edit');
$listUrl = asset($constant['secretRoute'].'/meeting-room/index');
$deleteUrl = asset($constant['secretRoute'].'/meeting-room/delete');
?>
		...
		<div class="panel">
			<div class="row panel-heading custome-panel-headering">
				<div class="form-group title-header-panel">
					<div class="pull-left">
						<div class="col-lg-12">{{trans('schedule.schedule')}} &raquo; {{trans('schedule.meeting_room')}}</div>
					</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$("#jqx-grid<?php echo $jqxPrefix;?>").jqxGrid({
				...
				columns: [
					{ text: '{{trans('schedule.meeting_room')}}', dataField: 'Name', width: '85%' },
					{ text: '{{trans('news.create_by')}}', dataField: 'user_name', width: '13%' }

				]
			});
			$("#btn-new").on('click',function(){
				newJqxItem('{{$jqxPrefix}}', '{{trans('trans.buttonNew')}}',500,180, '<?php echo $newUrl;?>', 0, '{{ csrf_token() }}');
			});

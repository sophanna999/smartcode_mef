<?php
$jqxPrefix = '_position';
$newUrl = asset($constant['secretRoute'].'/position/new');
$listUrl = asset($constant['secretRoute'].'/position/index');
$editUrl = asset($constant['secretRoute'].'/position/edit');
$deleteUrl = asset($constant['secretRoute'].'/position/delete');
?>
		...
		<div class="panel">
            <div class="row panel-heading custome-panel-headering">
                <div class="form-group title-header-panel">
                    <div class="pull-left">
                        <div class="col-lg-12 col-xs-12">{{trans('users.systemSetting')}} &raquo; {{trans('trans.position')}}</div>
                    </div>
...
<script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#jqx-grid<?php echo $jqxPrefix;?>").jqxGrid({
                    ...
                    columns: [
                        { text: '{{trans('trans.position')}}', dataField: 'NAME', width: '93%' },
						{text: 'លេខរៀង', dataField: 'ORDER_NUMBER', width: '5%' ,align:'center',cellsalign: 'center'}
                    ]
            });


            $("#btn-new<?php echo $jqxPrefix;?>").on('click',function(){
                newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonNew']}}',800,300, '<?php echo $newUrl;?>', 0, '{{ csrf_token() }}');
            });
            ...
            $("#btn-edit<?php echo $jqxPrefix;?>").on('click',function(){
                ...
                if(row.length == 0){
                    ...
                    newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonEdit']}}', 800,300, '<?php echo $editUrl;?>', jqxdatarow.ID, '{{ csrf_token() }}');
                }

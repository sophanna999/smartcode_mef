<?php
$jqxPrefix = '_commune';
$newUrl = asset($constant['secretRoute'].'/commune/new');
$editUrl = asset($constant['secretRoute'].'/commune/edit');
$listUrl = asset($constant['secretRoute'].'/commune/index');
$deleteUrl = asset($constant['secretRoute'].'/commune/delete');
?>
		...
		<div class="panel">
            <div class="row panel-heading custome-panel-headering">
                <div class="form-group title-header-panel">
                    <div class="pull-left">
                    <div class="col-lg-12">{{trans('users.systemSetting')}} &raquo; {{trans('trans.commune')}}</div>
                </div>
...
<script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#jqx-grid<?php echo $jqxPrefix;?>").jqxGrid({
                    ...
                    groups: ['name_kh_pro','name_kh_dis']
            });

           
            $("#btn-new<?php echo $jqxPrefix;?>").on('click',function(){
                newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonNew']}}',700,275, '<?php echo $newUrl;?>', 0, '{{ csrf_token() }}');
            });
            ...
            $("#btn-edit<?php echo $jqxPrefix;?>").on('click',function(){
                ...
                if(row.length == 0){
                    ...
                    newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonEdit']}}', 700, 275, '<?php echo $editUrl;?>', jqxdatarow.id, '{{ csrf_token() }}');
                }

<?php
$jqxPrefix = '_province';
$newUrl = asset($constant['secretRoute'].'/province/new');
$editUrl = asset($constant['secretRoute'].'/province/edit');
$listUrl = asset($constant['secretRoute'].'/province/index');
$deleteUrl = asset($constant['secretRoute'].'/province/delete');
?>
		...
		<div class="panel">
            <div class="row panel-heading custome-panel-headering">
                <div class="form-group title-header-panel">
                    <div class="pull-left">
                    <div class="col-lg-12">{{trans('users.systemSetting')}} &raquo; {{trans('trans.province')}}</div>
                </div>
...
<script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#jqx-grid<?php echo $jqxPrefix;?>").jqxGrid({
                    ...
                    columns: [
						{ text: 'ឈ្មោះអក្សរខ្មែរ', dataField: 'name_kh', width: '49%'},
						{ text: 'ឈ្មោះអក្សរឡាតាំង', dataField: 'name_en', width: '49%'  }                        
                    ]
            });

           
            $("#btn-new<?php echo $jqxPrefix;?>").on('click',function(){
                newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonNew']}}',700,180, '<?php echo $newUrl;?>', 0, '{{ csrf_token() }}');
            });
            ...
            $("#btn-edit<?php echo $jqxPrefix;?>").on('click',function(){
                ...
                if(row.length == 0){
                    ...
                    newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonEdit']}}', 700, 180, '<?php echo $editUrl;?>', jqxdatarow.id, '{{ csrf_token() }}');
                }

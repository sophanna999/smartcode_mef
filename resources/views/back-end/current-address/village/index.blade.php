<?php
$jqxPrefix = '_village';
$newUrl = asset($constant['secretRoute'].'/village/new');
$editUrl = asset($constant['secretRoute'].'/village/edit');
$listUrl = asset($constant['secretRoute'].'/village/index');
$deleteUrl = asset($constant['secretRoute'].'/village/delete');
?>
		...
		<div class="panel">
            <div class="row panel-heading custome-panel-headering">
                <div class="form-group title-header-panel">
                    <div class="pull-left">
                    <div class="col-lg-12">{{trans('users.systemSetting')}} &raquo; {{trans('trans.village')}}</div>
                </div>
...
<script type="text/javascript">
        ...
        var source<?php echo $jqxPrefix;?> =
        {
            ...
            dataFields: [
				...
				{ name: 'name_kh_dis', type: 'string' },
				{ name: 'name_kh_comm', type: 'string' }
            ],
			cache: false,
            id: 'id',
        ...
        $(document).ready(function () {
            ...
            $("#jqx-grid<?php echo $jqxPrefix;?>").jqxGrid({
                    ...
                    columns: [
						{ text: '{{trans('trans.province')}}', dataField: 'name_kh_pro', width: '20%', hidden:true},
						{ text: '{{trans('trans.district')}}', dataField: 'name_kh_dis', width: '20%', hidden:true},
						{ text: '{{trans('trans.commune')}}', dataField: 'name_kh_comm', width: '20%', hidden:true},
                    ...
                    groups: ['name_kh_pro','name_kh_dis','name_kh_comm']
            });

           
            $("#btn-new<?php echo $jqxPrefix;?>").on('click',function(){
                newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonNew']}}',700,322, '<?php echo $newUrl;?>', 0, '{{ csrf_token() }}');
            });
            ...
            $("#btn-edit<?php echo $jqxPrefix;?>").on('click',function(){
                ...
                if(row.length == 0){
                    ...
                    newJqxItem('<?php echo $jqxPrefix;?>', '{{$constant['buttonEdit']}}', 700, 322, '<?php echo $editUrl;?>', jqxdatarow.id, '{{ csrf_token() }}');
                }

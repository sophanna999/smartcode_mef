<?php
$jqxPrefix = '_village';
$saveUrl = asset($constant['secretRoute'].'/village/save');
$getDistrictByProId = asset($constant['secretRoute'].'/village/get-district-by-pro-id');
$getCommuneByDisId = asset($constant['secretRoute'].'/village/get-commune-by-dis-id');
?>
<div class="container-fluid">
    <form class="form-horizontal" role="form" method="post" name="jqx-form<?php echo $jqxPrefix;?>" id="jqx-form<?php echo $jqxPrefix;?>" enctype="multipart/form-data">
		...
		<div class="form-group">
            <div class="col-sm-3 text-right"><span class="red-star">*</span> {{trans('trans.commune')}}</div>
            <div class="col-sm-9">
				<input type="hidden" name="mef_commune_id" id="mef_commune_id" value="{{isset($row->mef_commune_id) ? $row->mef_commune_id:''}}">
...
<script>
	...
	function getCommuneByDisId(object){
		var mef_district_id = $(object).val();
        $.ajax({
            type: "post",
            url : '{{$getCommuneByDisId}}',
            datatype : "json",
            data : {"mef_district_id":mef_district_id,"_token":'{{ csrf_token() }}'},
            success : function(data){
               initDropDownList(jqxTheme, 485,30, '#div_mef_commune_id', data, 'text', 'value', false, '', '0', "#mef_commune_id","{{$constant['buttonSearch']}}",250);
            }
        });
	}
    $(document).ready(function(){
		...
		$('#div_mef_district_id').bind('select', function (event) {
            getCommuneByDisId(this);
        });
		/* Commune */
        initDropDownList(jqxTheme, 485,30, '#div_mef_commune_id', <?php echo json_encode($listCommune); ?>, 'text', 'value', false, '', '0', "#mef_commune_id","{{$constant['buttonSearch']}}",250);
		$('#jqx-form<?php echo $jqxPrefix;?>').jqxValidator({
            ...
            rules: [
				...
				{input: '#div_mef_commune_id', message: ' ', action: 'select',
                    rule: function () {
                        if($("#div_mef_commune_id").val() == ""){
                            return false;
                        }
                        return true;
                    }
